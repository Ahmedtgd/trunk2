...
class ProductSalesController extends Controller{
    ...
    public function productsales($uid=null){
        ...
        $merchant_id= DB::table('merchant')->
        where('user_id',$user_id)->pluck('user_id');
        ...
        return view('merchant.productsales',compact('since','locations'));
    }
    ...
    public function skulist_since(Request $request)
    {
        ...
        if($TimeFilter == 'YTD'){
			$where = " AND opos_receiptproduct.created_at BETWEEN '".
				date('Y-01-01 00:00:00')."' AND '".date('Y-m-d H:i:s')."'";

        }else if($TimeFilter == 'MTD'){
			$where = " AND opos_receiptproduct.created_at BETWEEN '".
				date('Y-m-01 00:00:00')."' AND '".date('Y-m-d H:i:s')."'";

        }else if($TimeFilter == 'WTD'){
			$weekDay = (date('D') != 'Mon') ?
				date('Y-m-d 00:00:00', strtotime('last Monday')) :
				date('Y-m-d 00:00:00');

			$where = " AND opos_receiptproduct.created_at BETWEEN '".
				$weekDay."' AND '".date('Y-m-d H:i:s')."'";

        }else if($TimeFilter == 'today'){
			$where =  " AND opos_receiptproduct.created_at BETWEEN '".
				date('Y-m-d 00:00:00')."' AND '".date('Y-m-d H:i:s')."'";
        }
        ...
        $totalsales = DB::select(DB::raw("
            ...
            SELECT
                ...
                -- format(SUM((opos_receiptproduct.price *
                -- opos_receiptproduct.quantity*(1 -
                -- (opos_receiptproduct.discount/100))))/100,2) as sales,

                ROUND(if(
                    
                    ops.mode = 'inclusive',
                SUM(
                    (opos_receiptproduct.price * opos_receiptproduct.quantity )/
                    (1 + ops.service_tax + opos_receiptproduct.servicecharge)
                )
                ,
                
                sum(opos_receiptproduct.quantity*opos_receiptproduct.price)
            ...
            FROM
                opos_receiptproduct
                JOIN opos_receipt as ops on ops.id =
    ...
    public function skulist_today(Request $request){
        ...
        if($difference >= 0) {
            ...
            $totalsales = DB::select(DB::raw("
				SELECT
				   ...
				   SUM((opos_receiptproduct.price*opos_receiptproduct.quantity*(1 - (opos_receiptproduct.discount/100))) - 
				   (1 - (ops.service_tax/100))) as sales,
				    SUM((opos_receiptproduct.quantity)) as sales_quantity,
				    ops.staff_user_id,
				    product.thumb_photo as image,
				    product.name as pname,
        ...
        "));
            ...
            foreach ($product as $key => $row) {
                ...
                $sales[$key]['max'] = $max;
                $sales[$key]['sales1'] = $row->sales;
                $sales[$key]['sales'] = number_format($row->sales / 100, 2);
                $sales[$key]['sales_quantity'] = $row->sales_quantity;
