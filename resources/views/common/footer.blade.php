...
if (empty($mode) or $mode=="osm") {
	# code...
	$mode="osm";
	$logo_url=asset('images/footer-logo.png');
	$home_url=route("home");
}
...
<footer class='ofooter nomobile' style="height:420px;background-color: #0a0909;">
    <div class="logo-header  text-center" >
        <div class="container">
            <div class="row">
				...
				@else
					<p style="margin-top:-10px;">
					&copy;2018 MPS Telecommunications Sdn. Bhd.. All Rights Reserved
					</p>
				@endif
				</div>
			</div>
...
-->
                ...
                <ul>
                    ...
                    <li>
						<a href="#" data-toggle="modal"
							...
							class="showregister" rel="buyer"> Buyer Registration</a>
					</li>
					@endif
					@if (Auth::check())
                    <li>
                    ...
                    @else
					<li>
                        <a href="#" data-toggle="modal" class="role_type_log" rel="merchant"
							data-target="#registerMerchantModal"> Merchant Registration </a>
					</li>
                    @endif
					...
					<li>
						<a href="#" data-toggle="modal"
							data-target="#loginModal"
							class="showregister"
							rel="station"> Station Registration</a>
					</li>
					@endif
					@if (Auth::check())
 					<li>
					...
					<li>
						<a href="javascript:void(0)"
						class="role_type_log show_fm"
						rel="fairmode"> Fair Mode Registration </a>
					</li>
					@endif
					@if (Auth::check())
  					<li>
					...
					<li>
						<a href="javascript:void(0)"
						class="role_type_log show_ep"
						rel="humancap"> Employee Benefit Registration </a>
					</li>
					@endif					
            ...
            <div class="col-sm-3 col-md-3">
				...
				{{--	<li><a href="{{route('openSupportIndex')}}">OpenSupport</a></li> --}}
					<!--
					<li><span style="color: #777;">OpenSupport</span></li>
					-->
				...
				<!--
				<li><span style="color: #777;">Download Apps</span></li>
				<li>{!! link_to_route('newsletter.index',"News") !!}</li>
				<li><span style="color: #777;">News</span></li>
				-->
		...
		<div style="position:relative;top:-200px;margin-bottom:-200px"
			class="row pull-right">
 			<div class="col-sm-3 col-md-3 text-right">
			<?php 
				$qr = DB::table('landingqr')->
				join('qr_management','qr_management.id','=','landingqr.qr_management_id')->
				orderBy('landingqr.id','DESC')->first();
			?>
				<table class="table">

				<tr style="border:none !important;">
					<td style="border:0;padding-right:4px;
						vertical-align:bottom">
					@if (Auth::check())
						<a href="javascript:void(0);" class="prevent">
							<button id="opossum" class="oposbtn">
								OPOSsum
							</button> 
						</a>
					@else
						<a href="/opossumLogin" data-toggle="modal">
							<button id="opossum" class="oposbtn">
								OPOSsum
							</button>
						</a>
					...
					<td style="border:none !important;
						...
						class=""
						style="height:70px;width:70px;margin-top:80px;padding-right:0;position:relative;left: 0px"/>
						</a> 
					</td>
				@if(!is_null($qr))
...
<span id="top-link-block" class="hidden thumbnail">
    <a href="#top" style="height:50px" class=" btn btn-lg btn-green"  onclick="$('html,body').animate({scrollTop:0},'slow');return false;">
...
<script type="text/javascript">
    $(document).ready(function(){
		...
		$('.opossumLogin').click(function(e){
			$("#opossumLoginModal").modal('show');
		});
		
        $('.role_type_log').click(function(e){
			var rel=$(this).attr('rel');
			$("#role_type_log").val(rel)
			if(rel == 'merchant'){
				$('.modal-inside-merchant').css('background-color', 'rgba(255,76,76,0.9)');
			} else if(rel == 'humancap'){
				$('.modal-inside-merchant').css('background-color', 'rgba(15, 113, 185,0.9)');
			} else if(rel == 'fairmode'){
				$('.modal-inside-merchant').css('background-color', 'rgba(204, 255, 102,0.9)');
			}
		});
                ...
                success:function(r){
                    ...
                    if (r.status=="success") {
						...
						$('#mogout').click(function(){
							...
							window.location.replace(JS_BASE_URL);
							$.ajax({
	...
	<div class="modal-content">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal"
				...
				<h3 class="modal-title" align="center" id="myModalLabel">
			...
			<div class="row" style="padding: 15px;">
				<div class="col-md-12" style="">
					...
					<div class="clearfix"></div><br>
					<center><a href="javascript:void(0);" class="btn btn-primary invs_btn"  style="width:100px;">Submit</a></center>
...
<script type="text/javascript">
	$(document).ready(function(){
		$('.about_us_show_as_modal').click(function(){
			var url =JS_BASE_URL+"/content";
			/*var w = window.open(url, "popupWindow",
				'width=550, height=850, toolbar=no, location=no, status=no, menubar=no, resizable=yes, scrollbars=yes');*/
...
if(url.indexOf('?show_login=true') != -1) {
