...
<div >
	...
	<div class="col-md-12"
		...
		<div class="col-md-4" style="position:relative;top:70px;padding-right:0">
			...
			<p style="margin: 0px">
				<?php if($buyeraddress->business_reg_no) echo "(".$buyeraddress->business_reg_no .")"?></p>
			...
			<p style="margin: 0px">
				...
				<p style="margin: 0px">
					<?php if($buyeraddress) if(isset($buyeraddress->postcode)){
						echo $buyeraddress->postcode;
					} ?>
				</p>
				<p style="margin: 0px">
					<?php if($buyeraddress) if(isset($buyeraddress->city)){
						echo $buyeraddress->city .', '.$buyeraddress->state;
					} ?>
				</p>
            ...
            @foreach($invoice as $invoice)
                ...
                <?php $price = $invoice->order_price;
						if(!is_null($invoice->approved_qty)){
							$invoice->quantity = $invoice->approved_qty;
						}
                $p_price = $price/100;
...
<script type="text/javascript">
  ...
  function showModal(id){
  	...
  	$.ajax({
			  type: "POST",
			  url: JS_BASE_URL+"/gator/SoConvertInvoice",
			  data:{"id":id},
			  success: function(data){
			  	console.log('***** showModal('+id+') *****');
			  	console.log('*****>'+data+'<*****');
			  	if(data.trim() == '0'){
			  		toastr.error('Sales Order has already been converted to invoice</br>Cancellation is not applicable.', { timeOut:5000 });
			  		$('#soModal').modal('hide');
			  	}else{
	  ...
	  // console.log(url);
		  ...
		  $.ajax({
			  ...
			  success: function(data){
			  	console.log(data);
			  	 if(data == 2){
			  	 	toastr.error('Unable to perform because the transaction is completed');
			  	// 	$('#confirmationModal').modal('hide');
			  	 }else if(data == 3){
					 toastr.error('Unable to perform because the transaction is cancelled');
				 }
