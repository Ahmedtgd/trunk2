    ...
    <div class="blur">
        ...
        <input hidden="" id="" type="text" value="{{$user_id}}" name="user_id">
    		...
    		<div  class="container">
    			...
    			<div style="background-color:green;float: right;color: white;padding-top: 17px;" type="button" id="newbuyerbtn" class="sellerbutton" data-toggle="newbuyer" data-target="#newbuyer">Unreg<br>Merchant</div>
				...
				Sales Order & Invoice Generator</h2>
    					...
    					<div class="container">
							...
							<?php
                                ...
                                foreach($products as $product){
									if($product->consignment_total>0 and
										$product->retail_price>0){
										++$new_index;
									}
								}
                                $index = 0; $count = 0 + $new_index; $save = $products ;?>
    							@foreach($products as $product)
                                <?php
								$tier_price =  DB::table('wholesale as w')->
                                join('product as p','p.id','=','w.product_id')->
                                where(['p.segment' => 'b2b','p.parent_id' => $product->parent_id])->
                                select('w.id','w.product_id','w.funit','w.unit','w.price')->
                                orderBy('w.price','desc')->get();

								//Count the array
								$array_count = count($tier_price);
								$last_index = 0;

								if(count($tier_price) > 0){
								   $contents = "" ;
									$contents .='<div class="tooltiptext row">';
									foreach ($tier_price as $t){
										++$last_index;
										$contents .='<div class="row">';

										if($array_count == $last_index){
											$contents .='<div style="padding:0;padding-left:10px" class="text-center col-xs-4">'.$t->funit.'&nbsp;+&nbsp;'.'</div>';
										}else{
											$contents .='<div style="padding:0;padding-left:10px" class="text-center col-xs-4">'.$t->funit.'&nbsp;-&nbsp;'.$t->unit.'</div>';
										}
										$contents .='<div style="padding:0;padding-right:15px" class="text-right col-xs-8">MYR&nbsp;'.number_format($t->price/100,2).'</div>';
										$contents .='</div>';
									}
									$contents .='</div>';
								}else{
        ...
        <td style="cursor:pointer;text-align: right;"  class='bmedium'>
									<span class="tooltips" id="price{{$index}}" data-placement="top" title="">
									{{number_format($product->retail_price/100,2)}} {!!$contents!!}
								</td>
         <span style="display: none;" id="priceget{{$index}}">{{$price/100}}</span>
             ...
             <td style="text-align: right;" class=' bmedium '><span class="total_of_goods" id="pricetotal{{$index}}" >{{number_format(0/100,2)}}</span></td>
		...
		<tr style="vertical-align: middle; background-color:lightgray;" class="{{$disable}}">
			<td style="visibility: hidden;">Test</td>
			<td class='text-center bmedium' >{{++$count}}</td>
			...
			<div style="width: 90%;float: right;">{{$product->name}}</div>
				...
				<td style="text-align: right;"  class='  bmedium'><span id="price{{$count}}"> {{number_format($price/100,2)}}</span></td>
					<span style="display: none;" id="priceget{{$count}}"> {{$price/100}}</span>
					<td>
						<div style="margin-left: 10%;" class="col-lg-10">
						<div class="input-group">
								<span class="input-group-btn">
									<button @if($product->consignment_total == 0 || $product->segment!='b2b') disabled="disabled"   @endif onclick="plus({{$count}},{{$product->id}},{{$product->consignment_total}})" type="button" class="quantity-right-plus btn-css btn btn-primary btn-number" data-type="plus" data-field="">
										<span class="glyphicon glyphicon-plus"></span>
									</button>
								</span>
								<span id="qtyfield{{$count}}">
										<input @if($product->consignment_total == 0 || $product->segment!='b2b') disabled="disabled"   @endif  onchange="qtyupdate({{$count}},{{$product->id}},{{$product->consignment_total}});"  type="text" id="d{{$count}}" name="quantity{{$product->id}}" class="form-control text-center input-number" value="0" min="0" max="1000000">
								</span>
								<span class="input-group-btn">
									<button @if($product->consignment_total == 0 || $product->segment!='b2b') disabled="disabled"   @endif onclick="minus({{$count}},{{$product->id}},{{$product->consignment_total}})" type="button" class="quantity-left-minus btn-css btn btn-primary btn-number"  data-type="minus" data-field="">
										<span class="glyphicon glyphicon-minus"></span>
									</button>
								</span>
						</div>
		...
		<td style="text-align: right;" class=' bmedium '>
			<span class="total_of_goods"  id="pricetotal{{$count}}" >{{number_format(0/100,2)}}</span>
		</td>
         </tr>
		<?php  }  ?>
         @endforeach
