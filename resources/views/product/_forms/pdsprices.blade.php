...
<div class="container">
    <div id="pinformation" class="row">
		...
		<input type="hidden" value="{{number_format($current_product->retail_price/100,2)}}" id="rPrice" name="rPrice" />
        ...
        <div class="col-sm-4 thumbnail">
            <div class="product-photo">
                <img class="img img-responsive"  id="preview-imgb2b"
					...
					src="{{asset('/')}}images/product/{{$pid}}/{{$current_product->photo_1}}"/>
            </div>
        </div>
        <div class="col-sm-8">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <p id="quantity_p">{{$current_product->available}}&nbsp;</p>
                </div>
            </div>
        </div>
    ...
    <div id="wholesale" class="row">
						...
						@if(!is_null($sprices))
							@foreach($sprices as $wholesaler)
								<tr class='wrow' data='{{$wp}}' id="wrow-{{$wp}}">
									...
									<?php
										$mysaveswr = 0;
										if($wholesaler->special_price > 0 && $current_product->retail_price > 0){
											$mysaveswr = (($current_product->retail_price - $wholesaler->special_price)/$current_product->retail_price)*100;
										}
										if($mysaveswr < 0){
											$mysaveswr = 0;
										}
        ...
        <div class="row">
            <div class="col-sm-12 text-right">
				<a href="javascript:void(0)" id="save_sp_product" class="btn btn-success pull-right"> Save Prices </a>
            </div>
        </div>
		<div class="row">
...
$(document).ready(function () {
	...
	$('#save_sp_product').click(function () {
		$("#save_sp_spinner").show();
		var fdata = new FormData($("#productRegsp")[0]);	
		$.ajax({
		  url: JS_BASE_URL + '/store_sp',
		  data:fdata,
		  dataType:'json',
		  async:false,
		  type:'post',
